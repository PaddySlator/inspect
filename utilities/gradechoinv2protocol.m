function protocol = gradechoinv2protocol(gradechoinv)
%
% Takes "gradechoinv" file and converts to camino.m type protocol file 
% in gradechoinv file, the first four columns are MRTrix-type grad file in format [x y z b],
% where x, y, z are the diffusion gradient directions and b is the b-value, 
% the fifth column is echo time, TE
% the sixth column is inversion time, TI
% the seventh column is repitition time, TR
%
% Paddy Slator (p.slator@ucl.ac.uk)

[bvec, bval, TE, TI, TR] = unpack_gradechoinv(gradechoinv);
...
% assign echo times, inversion times, and repitition times
protocol.TE = TE;
protocol.TI = TI;
protocol.TR = TR;
...
% make the gradient directions for b=0's [0 0 0]
for i=1:length(protocol.b0_Indices)
    protocol.grad_dirs(protocol.b0_Indices(i),:) = [0 0 0];
end
